...
class PointtClient {
  ...
  static async authenticate(email, password) {
    ...
    // Helper: given a landed page, handle the password step of a multi-step login
    const handlePasswordPage = async (html, currentUrl) => {
      const step2Form = /<form[^>]+method="post"[^>]*>/i.exec(html) ||
                        /<form[^>]+>/i.exec(html);
      const step2Action = step2Form ? attr(step2Form[0], 'action') : null;
      const step2PostUrl = step2Action
      ...
      const step2PwdMatch = html.match(/<input[^>]+type="password"[^>]*/i);
      const step2PwdField = (step2PwdMatch && attr(step2PwdMatch[0], 'name')) || 'Input.Password';

      console.log(`[PointtAuth] Step 2 password POST → ${step2PostUrl.slice(0, 100)} field="${step2PwdField}"`);
      ...
      if (step2Res.status >= 300 && step2Res.status < 400 && step2Res.headers.location) {
        ...
        // Check if it's another step (unlikely)
        console.log(`[PointtAuth] Step 2 landed on page: ${r2.url.slice(0, 100)}`);
      }
      // 200 on step 2 POST = wrong password
      ...
      throw new Error('Login did not complete after password step.');
    };
    ...
    if (postRes.status >= 300 && postRes.status < 400 && postRes.headers.location) {
      ...
      if (result.type === 'page' && result.body.includes('type="password"')) {
